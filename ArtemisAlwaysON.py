...
# Set confirmation to always on
pig_detection_confirmed = True
print("Pig detection confirmation set to ALWAYS ON")
...
while True:
    ...
    # Extract detection data - this is still used for display purposes
    ...
    # Always show detection as confirmed since pig_detection_confirmed is always True
    status = "DETECTION: PIG CONFIRMED (Auto Mode)"
    color = (0, 255, 0)

    cv2.putText(stacked_frames, status, (10, stacked_frames.shape[0] - 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

    # Add detection details without affecting confirmation
    if usb_detects_pig:
        cv2.putText(stacked_frames, "USB camera: Pig detected", (stacked_frames.shape[1] // 2 - 300, stacked_frames.shape[0] - 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
    if thermal_detects_pig:
        cv2.putText(stacked_frames, "Thermal camera: Pig detected", (stacked_frames.shape[1] // 2 - 300, stacked_frames.shape[0] - 100),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
    ...
    if key == ord(' '):
        print("Space bar pressed - playing sound (always confirmed)")
        pygame.mixer.music.play()
        print("Playing sound: Pig always confirmed")
    elif key == ord('q'):
        ...
        print(f"Detection mode: Always confirmed")
